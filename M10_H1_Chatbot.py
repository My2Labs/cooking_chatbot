...
def clean_target_for_eval(text):
    text = text.replace(START_TOKEN, "").replace(END_TOKEN, "").strip()
    text = re.sub(r"\s+([,.?!:/()'\-])", r"\1", text)
    return text

def evaluate_rouge(test_pairs, max_examples=200):
    bot = ChatBot()
...
results, results_df = evaluate_rouge(test_pairs, max_examples=200)
...
print("\nSample Predictions: ")
print(results_df[["reference", "prediction", "rougeL_f"]].head(10))
